...
VERSION = "1.2.0" # Versión actual del script (con xdotool)
...
class WebUIKeyboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _type_text(self, text):
        """Funcion central para simular la escritura, usando xdotool para acentos y pyautogui.press() para otros."""
        if not text:
            print("Nada que escribir.", file=sys.stderr)
            return
        
        print(f"Escribiendo texto: {text[:100]}... (usando xdotool para acentos)", file=sys.stderr)

        # Mapeo de caracteres acentuados a su base para xdotool con ISO_Level3_Shift
        xdotool_map = {
            'á': 'a', 'é': 'e', 'í': 'i', 'ó': 'o', 'ú': 'u',
            'Á': 'A', 'É': 'E', 'Í': 'I', 'Ó': 'O', 'Ú': 'U',
            'ñ': 'n', 'Ñ': 'N',
        ...
        for char in text:
            if char in xdotool_map:
                try:
                    subprocess.run(['xdotool', 'key', '--clearmodifiers', f"ISO_Level3_Shift+{xdotool_map[char]}"], check=True)
                except subprocess.CalledProcessError as e:
                    print(f"Error xdotool para '{char}': {e}", file=sys.stderr)
                    pyautogui.press(char) # Fallback a pyautogui si xdotool falla
            else:
                pyautogui.press(char) # Usar pyautogui para caracteres normales
